...
public void simularFSJ(String politica, ArrayList<Proceso> listaDeProcesos, ConfiguracionSistema configuracion){
...
private Proceso asignarProcesoConRafagaCPUMasCorta() {
    ...
    if(llena){
        System.out.println("LLEGARON TODOS LOS PROCESOS");
        procesoActual = Collections.min(Proceso.listaEjecutando, Comparator.comparingInt(p -> p.getDuracionRafagaCPU()));
    }else{
        System.out.println("NO LLEGARON TODOS LOS PROCESOS");
        for(Proceso p :Proceso.listaEjecutando){
           if(tiempoActual == p.getTiempoArribo()){
                procesoActual= p;
            }
