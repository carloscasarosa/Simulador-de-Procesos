...
public void simularSRT(String politica, ArrayList<Proceso> listaDeProcesos, ConfiguracionSistema configuracion){
    ...
    Thread simulacionThread = new Thread(() -> {
        ...
        while(procesosTerminados < Proceso.listaDeProcesos.size()){
           ...
           while(!Proceso.listaEjecutando.isEmpty() && procesoActual.getRafagaCPUrestante() >0){ 
               p = procesoActual;                      //LO UTILIZO PARA VERIFICAR EL CAMBIO DE CONTEXTO MAS ADELANTE
                ...
                if(procesoActual.getRafagaCPUrestante() == 0){   //SI EJECUTO LAS RAFAGAS DE CPU 
                    ...
                    procesoActual.setRafagasCPU(rafagasCPU);
                     ...
                     if(rafagasCPU >0){
                        ...
                        tiempoActual++;
                        procesoActual=asignarProcesoConRafagaCPUMasCorta(); 
                        ...
                        if(!Proceso.listaEjecutando.isEmpty()){
                          procesoActual=asignarProcesoConRafagaCPUMasCorta();
                        }
                    }
...
private Proceso asignarProcesoConRafagaCPUMasCorta() {
    ...
    if(llena){
        procesoActual = Collections.min(Proceso.listaEjecutando, Comparator.comparingInt(p -> p.getRafagaCPUrestante()));
    }else{
        for(Proceso p :Proceso.listaEjecutando){
             if(tiempoActual == p.getTiempoArribo()){
                procesoActual= p;
...
private void verificarRafagasES(DefaultTableModel modelo) {
    ...
    if (procesoActual.getDuracionRafagaES() >0){ 
        System.out.println("INGRESA A EJECUTAR RAFAGA DE E/S "  + " " + tiempoActual);                     
         //CAMBIA EL ESTADO 
